...
class _ModelNet40Dataset_registration(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # else:
            ...
            # normals = pc_normal[idx, 3:].astype(np.float32) # n, 3
        #print(points.shape)
        points = pc_normal[:, :3].astype(np.float32) # n, 3
        normals = pc_normal[:, 3:].astype(np.float32) # n, 3
        if self.normalize:
        ...
        if self.with_normals:
            # _, points, normals = random_rotation(points, normals, max_degree=self.max_degree, max_amp=self.max_amp, seed=0)
            trans, trans_points, trans_normals = random_rotation(points, normals, max_degree=self.max_degree, max_amp=self.max_amp, seed=1)
            pcd = np.concatenate((points, normals), axis=1)
            trans_pcd = np.concatenate((trans_points, trans_normals), axis=1)
            #print(f'trans={trans}')
        else:
            # _, points = random_rotation(points, max_degree=self.max_degree, max_amp=self.max_amp, seed=0)
            trans, trans_points = random_rotation(points, max_degree=self.max_degree, max_amp=self.max_amp, seed=1)
            pcd = points
            trans_pcd = trans_points
        #print(pcd)
        if self.p_keep:
            
            pcd = self.project(pcd, 200) # 200*200
            trans_pcd = self.project(trans_pcd, 200)
            #print(pcd.shape)
            # pcd1 = vt.visualize_pcd(pcd[:, :3], vt.FRAG1_COLOR)
            # pcd2 = vt.visualize_pcd(trans_pcd[:, :3], vt.FRAG2_COLOR)
        ...
        if self.add_noise:
            if self.clip:
                noise1 = np.clip(0.01 * np.random.randn(pcd.shape[0], 3), -1*self.clip, self.clip)
                noise2 = np.clip(0.01 * np.random.randn(trans_pcd.shape[0], 3), -1*self.clip, self.clip)
            else:
                noise1 = 0.01 * np.random.randn(pcd.shape[0], 3)
                noise2 = 0.01 * np.random.randn(trans_pcd.shape[0], 3)
            pcd[:, :3] += noise1
            trans_pcd[:, :3] += noise2
        #print('sample')
        #print(pcd)
    ...
    def glob_dataset(root, filenametxt, class_to_idx):
        ...
        return samples
    def project(self, points, grid_num):
        centroid = np.mean(points[:, :3], axis=0)
        points_centered = points[:, :3] - centroid
        # bound in a [-1, 1]box
        xyz_max = np.max(points_centered, axis=0)
        xyz_min = np.min(points_centered, axis=0)
        points_centered_bound = 2*(points_centered-xyz_min)/(xyz_max - xyz_min)
        points_centered_bound_xy = points_centered_bound[:, :2]
        grid_size = 2/grid_num #2/200
        grid_xy = np.floor(points_centered_bound_xy/grid_size).astype(np.int32)
        grid_id = grid_xy[:, 0] + grid_xy[:, 1]*grid_num
        #print(f'grid_xy[:, 0]={grid_xy[grid_id>40000, 0]}, grid_xy[:, 1]={grid_xy[grid_id>40000, 1]}, grid_id={grid_id}')
        minz = 10000*np.ones((grid_num+1)*(grid_num+1))
        ids = -1*np.ones((grid_num+1)*(grid_num+1), np.int32)
        partial_points = []
        for i in range(points_centered_bound.shape[0]):
            #print(points_centered_bound[i, 2],minz[grid_id[i]])
            if points_centered_bound[i, 2] < minz[grid_id[i]]:
                #print(f'pcb={points_centered_bound[i, 2]}')
                #print(f'bminz={minz[grid_id[i]]}')
                minz[grid_id[i]] = points_centered_bound[i, 2]
                #print(f'aminz={minz[grid_id[i]]}')
                ids[grid_id[i]] = i
        #print(minz[minz<10000])
        #print(ids)
        for id in ids:
            if id is not -1:
                partial_points.append(points[id])
        return np.stack(partial_points, axis=0)
    ...
    def update(self, output:torch.Tensor, target:torch.Tensor):
        with torch.no_grad():
            ...
            for i in range(pt1.shape[0]):
                ...
                # o3d.visualization.draw_geometries([pcd1, pcd2])
                # print("visual after")
                # pcd1 = vt.visualize_pcd(est_trans_pt, vt.FRAG1_COLOR)
                # pcd2 = vt.visualize_pcd(gt_trans_pt, vt.FRAG2_COLOR)
                # o3d.visualization.draw_geometries([pcd1, pcd2])
