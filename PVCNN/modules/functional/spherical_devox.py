...
__all__ = ['spherical_trilinear_devoxelize']


class Spherical_TrilinearDevoxelization(Function):
    @staticmethod
    def forward(ctx, features, coords, g_inds, resolution, is_training=True):
        """
        :param ctx:
        ...
        outs, inds, wgts = _backend.spherical_trilinear_devoxelize_forward(resolution, is_training, coords, features, g_inds)
        #print(is_training)
        #if is_training:
    ...
    def backward(ctx, grad_output):
        ...
        grad_inputs = _backend.spherical_trilinear_devoxelize_backward(grad_output.contiguous(), inds, wgts, ctx.r)
        #print(grad_inputs.shape)
        return grad_inputs.view(grad_output.size(0), grad_output.size(1), ctx.r, ctx.r, ctx.r), None, None, None, None


spherical_trilinear_devoxelize = Spherical_TrilinearDevoxelization.apply
