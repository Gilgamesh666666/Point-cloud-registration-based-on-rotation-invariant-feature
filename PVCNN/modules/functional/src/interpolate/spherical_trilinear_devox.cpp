#include "spherical_trilinear_devox.hpp"
#include "spherical_trilinear_devox.cuh"
...
spherical_trilinear_devoxelize_forward(const int r, const bool is_training,
                             const at::Tensor coords,
                             const at::Tensor features,const at::Tensor g_inds) {
  CHECK_CUDA(features);
  CHECK_CUDA(coords);
  ...
  //if (is_training) {
    ...
    spherical_trilinear_devoxelize(b, c, n, r, r2, r3, true, coords.data_ptr<float>(),
                         features.data_ptr<float>(), g_inds.data_ptr<int>(), inds.data_ptr<int>(),
  ...
  //   spherical_trilinear_devoxelize(b, c, n, r, r2, r3, false, coords.data_ptr<float>(),
  //                        features.data_ptr<float>(), g_inds.data_ptr<int>(), inds.data_ptr<int>(),
  //                        wgts.data_ptr<float>(), outs.data_ptr<float>());
  //   return {outs, inds, wgts};
  // }
...
at::Tensor spherical_trilinear_devoxelize_backward(const at::Tensor grad_y,
  ...
  spherical_trilinear_devoxelize_grad(b, c, n, r3, indices.data_ptr<int>(),
