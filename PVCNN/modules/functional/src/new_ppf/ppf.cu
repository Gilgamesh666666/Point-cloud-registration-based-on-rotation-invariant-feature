...
*/
__global__ void compare_alpha_angle_kernel(int b, int n, 
  ...
  for(int i = index;i < n;i += stride)
  {
    ...
    else
    {
      ...
      float apha1 = dx*cnx+dy*cny+dz*cnz;
      float apha2 = dx*nx+dy*ny+dz*nz;
      float apha3 = cnx*nx+cny*ny+cnz*nz;
